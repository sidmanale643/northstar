-- Migration 011: Repair the API-key RPC result names and restrict execution

DROP FUNCTION public.create_or_rotate_project_api_key(UUID, TEXT, UUID, TEXT);

CREATE FUNCTION public.create_or_rotate_project_api_key(
    p_project_id   UUID,
    p_project_name TEXT,
    p_key_id       UUID,
    p_key_hash     TEXT
)
RETURNS TABLE (result_project_id UUID, result_created_at TIMESTAMPTZ)
LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION public.create_or_rotate_project_api_key(UUID, TEXT, UUID, TEXT) FROM anon;
REVOKE ALL ON FUNCTION public.create_or_rotate_project_api_key(UUID, TEXT, UUID, TEXT) FROM authenticated;
